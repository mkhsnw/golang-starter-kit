...
OPTIONAL FLAGS:
  ...
  --uuid                Use UUID (string, VARCHAR(36)) as primary key instead
                        of auto-increment BIGINT. UUID is generated by the app.
...
EXAMPLES:
  ...
  task gen name=Invoice fields="amount:float64,note:text?" uuid=true
...
func runModuleGenerator(args []string) {
	...
	var force, dryRun, useUUID, runMigrate bool
	...
	mod := buildModuleNames(name, fieldsStr, useUUID)
...
type ModuleNames struct {
	...
	Fields  []Field
	UseUUID bool    // true if --uuid flag is passed
}

func buildModuleNames(input, fieldsStr string, useUUID bool) ModuleNames {
		...
		for _, part := range parts {
			...
			if len(kv) == 2 {
				...
				fields = append(fields, Field{
					...
					IsForeignKey: strings.HasSuffix(toSnakeCase(name), "_id"),
				})
			}
		}
	}
